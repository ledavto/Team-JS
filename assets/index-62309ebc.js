import{a as f,P as v}from"./go-top-4da61b63.js";let i=1,s="Body parts",o,p="",u="";function g(t,e=1){window.innerWidth<768?o=9:o=12,f.getFilters({filter:t,page:e,limit:o}).then(d=>{const l=document.getElementById("category-cards");l.innerHTML="",d.results.forEach(n=>{const a=document.createElement("div");a.style.background=`linear-gradient(0deg, rgba(17, 17, 17, 0.50) 0%, rgba(17, 17, 17, 0.50) 100%), url(${n.imgURL}) lightgray -22.462px -5px / 109.595% 107.556% no-repeat`,a.classList.add("category-card"),a.innerHTML=`<h3 class="category-title">${n.name}</h3>
        ...
        `,y.appendChild(m),m.addEventListener("click",x=>{console.log("event.currentTarget: ",x.currentTarget),document.querySelector(".backdrop-modal").classList.remove("is-hidden")}),document.querySelector(".modal-general-close-btn").addEventListener("click",document.querySelector(".backdrop-modal").classList.add("is-hidden"))});else{const r=document.createElement("div");r.classList.add("exr-empty-search-list"),r.innerHTML="Nothing was found for your request.",y.appendChild(r)}L(a.totalPages),document.querySelector(".exercise-section-title").innerHTML=`Exercise / <span class="exercise-section-title-span">${e}</span>`,document.querySelector(".exercise-section-search").style.display=""}).catch(a=>{console.error(a)})}document.querySelector(".exercise-section-search-form").addEventListener("submit",t=>{t.preventDefault();const e=t.target;u=e.elements.keyword.value.trim(),i=1,e.elements.keyword.value="",h(s,p,u,i)});function L(t){const e=document.getElementById("pagination");e.innerHTML="",t>0?e.style.display="":e.style.display="none";const c={totalItems:t*o,itemsPerPage:o,visiblePages:5,page:i};new v(e,c).on("beforeMove",l=>{const n=l.page;i=n,h(s,p,u,n)})}
