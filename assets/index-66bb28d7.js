import{a as x,s as v,m as b,P as w,n as h}from"./go-top-742b48e5.js";let i=1,a="Body parts",d,y="",p="";function g(t,e=1){window.innerWidth<768?d=9:d=12,x.getFilters({filter:t,page:e,limit:d}).then(l=>{const s=document.getElementById("category-cards");s.innerHTML="",l.results.forEach(n=>{const o=document.createElement("div");o.style.background=`linear-gradient(0deg, rgba(17, 17, 17, 0.50) 0%, rgba(17, 17, 17, 0.50) 100%), url(${n.imgURL}) top / cover no-repeat`,o.classList.add("category-card"),o.innerHTML=`<h3 class="category-title">${n.name}</h3>
        ...
        `,f.appendChild(u),u.addEventListener("click",async()=>{const L=u.getAttribute("data-id");document.querySelector(".modal-general").setAttribute("data-id",L),await b.setDataExerciseModal(L)})});else{const c=document.createElement("div");c.classList.add("exr-empty-search-list"),c.innerHTML="Nothing was found for your request.",f.appendChild(c)}q(o.totalPages),document.querySelector(".exercise-section-title").innerHTML=`Exercise / <span class="exercise-section-title-span">${e}</span>`,document.querySelector(".exercise-section-search").style.display=""}).catch(o=>{console.error(o)})}document.querySelector(".exercise-section-search-form").addEventListener("submit",t=>{t.preventDefault();const e=t.target;p=e.elements.keyword.value.trim(),i=1,e.elements.keyword.value="",E(a,y,p,i)});function q(t){const e=document.getElementById("pagination");e.innerHTML="",t>0?e.style.display="":e.style.display="none";const r={totalItems:t*d,itemsPerPage:d,visiblePages:5,page:i};new w(e,r).on("beforeMove",s=>{const n=s.page;i=n,E(a,y,p,n)})}const m=document.getElementById("subscribe-input"),S=async t=>{t.preventDefault();const e=m.value,r=/^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*$/;if(e.match(r)){const s=await x.subscribe(e);s===void 0||s.status!==201&&s.status!==200?(m.classList.add("error"),h.Notify.failure(s.data.message)):(m.classList.remove("error"),m.value="",h.Notify.success(s.data.message))}else{m.classList.add("error");const s=e===""?"Email can not be empty!":"Invalid email!";h.Notify.failure(s)}};document.getElementById("subscribe-btn").addEventListener("click",S);
